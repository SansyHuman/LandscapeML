...
def build_data(sampler: TheorySampler, autoencoder: nn.Module, device: torch.device, n_per_theory: int, n_iter: int, grid: np.ndarray, kde_bandwidth: float):
    ...
    for i in range(n_iter):
        ...
        for row in rows:
            data_per_theories[row["Name"]][i].append(SuperConformalIndex(row["SCI"]).featurize_sci(grid, kde_bandwidth)[:-16])
    ...
    with torch.no_grad():
        for theory in theories:
            for i in range(n_iter):
                data_per_theories[theory][i] = torch.tensor(np.stack(data_per_theories[theory][i]), dtype=torch.float32).to(device)
                data_per_theories[theory][i] = autoencoder(data_per_theories[theory][i]).detach().cpu().numpy()
...
if __name__ == '__main__':
    ...
    def pair_calculation(theory1: str, theory2: str):
        ...
        print(f"Stats for {theory1} and {theory2} with autoencoder clustering")
