...
if __name__ == '__main__':
    ...
    def triplet_calculation(theories: tuple[str, str, str]):
        ...
        print(f"Stats for {theories} with sci clustering")
    ...
    plt.savefig(f"{save_dir}/sci_cluster_triplet.png")
    plt.show()

    accuracy = accuracy.tolist()
    stdev = stdev.tolist()

    with open(f"{save_dir}/sci_cluster_triplet_acc.csv", 'w', newline='') as csv_file:
    ...
    with open(f"{save_dir}/sci_cluster_triplet_stdev.csv", 'w', newline='') as csv_file:
        writer = csv.writer(csv_file)
        for i in range(n_theory):
            writer.writerow([theories[i]] + theories)
            for j in range(n_theory):
